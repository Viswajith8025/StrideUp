...
export function AppShell({ children, showNav = true, showHeader }: AppShellProps) {
  ...
  return (
    <div className="app-frame min-h-screen text-foreground safe-top">
      <div className="app-column relative">
        {displayHeader && (
          <header className="sticky top-0 z-40 border-b border-border/60 bg-background/90 backdrop-blur-md">
            <div className="flex items-center justify-end px-4 py-3">
              <Link
                href="/notifications"
                className="pressable relative inline-flex h-11 w-11 items-center justify-center rounded-full hover:bg-card"
        ...
        <main className={showNav ? "pb-[calc(var(--nav-height)+env(safe-area-inset-bottom))]" : ""}>
          <div className="px-4">
            {showNav ? <PageTransition>{children}</PageTransition> : children}
          </div>
        </main>

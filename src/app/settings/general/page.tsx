...
export default function GeneralSettingsPage() {
  ...
  useEffect(() => {
    if (profile) setGoal(String(profile.daily_step_goal));
    if (settings) {
      setDistanceUnit(settings.distance_unit);
      setWeightUnit(settings.weight_unit);
      setWeekStart(String(settings.week_starts_on));
    }
  }, [profile, settings]);
    ...
    <AppShell showNav={false}>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <select value={distanceUnit} onChange={(e) => setDistanceUnit(e.target.value)} className="w-full h-12 rounded-xl bg-card border border-border px-4">
        ...
        <div>
          ...
          <select value={weightUnit} onChange={(e) => setWeightUnit(e.target.value)} className="w-full h-12 rounded-xl bg-card border border-border px-4">
